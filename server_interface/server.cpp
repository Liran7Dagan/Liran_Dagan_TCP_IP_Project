...
int main()
{
	...
	if (!initialize()) {
		cout << "Failed to open the server." << WSAGetLastError() << endl;
		return -1;
	}
	...
	// step 3.1: Server Address struct - here we set the server's ip and port number
	...
	// step 3.2: Bind the TCP type socket from earlier to the address struct (ip, port number) 
	if (!assignAddress(listenSocket, serverAddr))
	{
