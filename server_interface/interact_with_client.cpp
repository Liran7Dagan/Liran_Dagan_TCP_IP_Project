...
bool isQuit(string& message, ClientInfo& client)
{
	...
	{
		send(client.socket, quitProgram.c_str(), quitProgram.length(), 0);
	...
	*/
	...
	while (true) {
		...
		case 2: // Message target client by forwarding ('send') client's message (client -> server (we're here) -> target)
			...
			ClientInfo* sendTo = findByName(targetName, all_clients); // Check if target hasn't disconnected
			if (sendTo == NULL) {
				cout << "'" << client.name << "'s connection with '" << targetName << "' has been terminated." << endl;
				string DCerror = "S:TargetDC";
				send(client.socket, DCerror.c_str(), DCerror.length(), 0);
				mode = 1;
			}
			else
			{
				// Why add "C:"? See 'isError' function's definition note 
				...
				if (send(sendTo->socket, display.c_str(), display.length(), 0) == SOCKET_ERROR)
					cout << "'" << client.name << "' Failed to send message to '" << sendTo->name << "'" << endl;
				else
